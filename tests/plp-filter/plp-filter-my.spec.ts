...
// ==========================================
// 3. TARGET URL LIST (DIRECT NAVIGATION MALAYSIA)
// ==========================================
const targetPages = [
    { name: 'Accessories', url: 'https://www.americanstandard.com.my/bathrooms/accessories/' },
    { name: 'Basin Faucets', url: 'https://www.americanstandard.com.my/bathrooms/basin-faucets/' },
    { name: 'Bath Showers', url: 'https://www.americanstandard.com.my/bathrooms/bath-showers/' },
    { name: 'Commercial', url: 'https://www.americanstandard.com.my/bathrooms/commercial/' },
    { name: 'Spalets', url: 'https://www.americanstandard.com.my/bathrooms/spalets/' },
    { name: 'Tapwares', url: 'https://www.americanstandard.com.my/bathrooms/tapwares/' },
    { name: 'Toilets', url: 'https://www.americanstandard.com.my/bathrooms/toilets/' },
    { name: 'Urinal', url: 'https://www.americanstandard.com.my/bathrooms/urinal/' },
    { name: 'Vanities', url: 'https://www.americanstandard.com.my/bathrooms/vanities/' },
    { name: 'Wash Basins', url: 'https://www.americanstandard.com.my/bathrooms/wash-basins/' },
    { name: 'Kitchen Faucets', url: 'https://www.americanstandard.com.my/kitchens/kitchen-faucets/' }
];
...
async function dismissCookies(page: Page) {
    const cookieButton = page.getByRole('button', { name: /Accept|Got it|Agree|Allow/i }).first();
    try {
...
// ==========================================
test.describe('PLP Filter Dependencies - Direct URL (Malaysia)', () => {
    ...
    for (const target of targetPages) {
        ...
        test(`Validate dynamically available Styles for: ${target.name}`, async ({ page }, testInfo) => {
            ...
            for (let i = 0; i < styleCount; i++) {
                ...
                let styleNameMyUI = styleNameRaw?.trim() || '';

                if (!styleNameMyUI) continue;

                // Normalize "Other" or "Others" variations
                if (styleNameMyUI.toLowerCase() === 'other' || styleNameMyUI.toLowerCase() === 'others') {
                    styleNameMyUI = 'Others';
                }

                await test.step(`Check Collections for available Style: "${styleNameMyUI}"`, async () => {
                    ...
                    for (const visibleCollection of actualCollections) {
                        ...
                        const expectedStyleMy = styleTranslationMap[expectedStyleEn];

                        // Assertion 2: Ensure collection belongs to the currently clicked Style
                        expect.soft(
                            expectedStyleMy,
                            `[Bug Mismatch!] Collection "${visibleCollection}" (expected: ${expectedStyleMy}) incorrectly appeared under the "${styleNameMyUI}" filter! \n🔗 Error URL: ${currentUrl}`
                        ).toBe(styleNameMyUI);
                    }
