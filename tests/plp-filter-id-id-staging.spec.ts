...
const styleTranslationMap: Record<string, string> = {
    ...
    "Classic": "Klasik",
    "Others": "Others" // Adjusted based on your staging site's localization
};

// ==========================================
// 3. TARGET URL LIST (DIRECT NAVIGATION STAGING INDONESIA)
// ==========================================
const targetPages = [
    { name: 'Area Komersial', url: 'https://id.47.130.209.149.nip.io/id/bathrooms/area-komersial/' },
    { name: 'Bathtubs', url: 'https://id.47.130.209.149.nip.io/id/bathrooms/bathtubs/' },
    { name: 'Keran Bath & Shower', url: 'https://id.47.130.209.149.nip.io/id/bathrooms/keran-bath-shower/' },
    { name: 'Keran Wastafel', url: 'https://id.47.130.209.149.nip.io/id/bathrooms/keran-wastafel/' },
    { name: 'Spalet', url: 'https://id.47.130.209.149.nip.io/id/bathrooms/spalet/' },
    { name: 'Toilets', url: 'https://id.47.130.209.149.nip.io/id/bathrooms/toilets/' },
    { name: 'Aksesoris', url: 'https://id.47.130.209.149.nip.io/id/bathrooms/aksesoris/' },
    { name: 'Vanity', url: 'https://id.47.130.209.149.nip.io/id/bathrooms/vanity/' },
    { name: 'Wastafel', url: 'https://id.47.130.209.149.nip.io/id/bathrooms/wastafel/' }
];
...
async function dismissCookies(page: Page) {
    ...
    try {
        // Set timeout short (1s) so it doesn't waste time if the banner isn't there
...
// ==========================================
test.describe('PLP Filter Dependencies - Direct URL (Indonesia Staging)', () => {
        ...
        test(`Validate dynamically available Styles for: ${target.name}`, async ({ page }, testInfo) => {
            ...
            for (let i = 0; i < styleCount; i++) {
                ...
                await test.step(`Check Collections for available Style: "${styleNameIdUI}"`, async () => {
                    ...
                    // 1. CLICK STYLE
                    await dismissCookies(page); // Clear cookies that might have popped up
                    ...
                    await dismissCookies(page); // Clear cookies that might have popped up again
                    await currentStyleLabel.click();
